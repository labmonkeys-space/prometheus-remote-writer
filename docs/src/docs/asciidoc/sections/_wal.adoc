...
* `batch.linger-ms` and `queue.store-policy` are *ignored* — the WAL flusher
  sends what the reader returns and a full WAL refuses every later append. A
  WARN is logged at startup if either is set.
