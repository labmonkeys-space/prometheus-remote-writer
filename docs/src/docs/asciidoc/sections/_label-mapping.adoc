...
the snake_case names disappear from the wire. The recipe is validated
against the reserved-target rules in <<reserved-rename-copy-targets>> —
none of the camelCase targets collide.
...
| `ifSpeed`       | restored *by name*, *one* label | *Value differs for high-speed interfaces.* Cortex emitted both `ifSpeed` and `ifHighSpeed` raw; this plugin emits a single normalized value: `ifHighSpeed × 1_000_000` when non-zero, else `ifSpeed`. Dashboards that read `ifSpeed` for non-high-speed interfaces see no change; for high-speed interfaces they now get a bps value, not Mbps. There is no `ifHighSpeed` label to recover.
