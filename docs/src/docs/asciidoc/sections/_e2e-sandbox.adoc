...
metadata** dashboard: a data series legended by the
`group_left` join on `onms_resource_info`, the attribute rows and
categories of one resource, and the label-name count (see
<<resource-metadata>>).
...
* the fleet (`e2e/tools/smoke-metadata.sh`): the `e2e` requisition is imported over REST and the snmpd node's interfaces collected; then `onms_resource_attr` carries `ifDescr=eth0`, `onms_resource_category` carries `Routers` and `Production`, `onms_resource_info` carries `if_descr=eth0` and `onms_resource_ifspeed` is 10,000,000,000 bit/s for it; the documented `group_left` join, the utilisation divisor and the category membership join each return a series, and the `@ end()` idiom evaluates on the backend;
