...
- **BREAKING: the v0.x attribute labels are gone from the data series. 1.0.0 is a cutover, not a migration** (epic #189: #190, #191, #192, #193). Resource string attributes, surveillance categories and the interface speed now travel as series of their own: `onms_resource_attr{resourceId, key, value} 1`, `onms_resource_category{resourceId, category} 1`, `onms_resource_info{resourceId, <columns>} 1` and the gauge `onms_resource_ifspeed{resourceId}` in bits per second. The data series lose `if_descr`, `if_speed`, `ifSpeed`, `ifHighSpeed`, `categories`, `onms_cat_*`, `onms_attr_*` and `onms_extattr_*`; `if_name` is the only interface label left, because `ifName` is the one interface field no operator sets, and an operator editing an interface description must not split a year of throughput history. That changes the identity of every affected series once, at cutover. Series written before it keep their old identity for the retention window and are not joined to the new ones. Dashboards that filtered on a removed label move to the `group_left` join documented under Resource metadata. OpenNMS graph rendering is unaffected across the boundary: the read path resolves resources by `__name__` and `resourceId`, both of which stay, and restores `${…}` placeholders from the rows. No compatibility mode, no dual emission, no read-path support for the old prefixes.
  The keys that shaped the removed labels are gone with them: `labels.attr-mode`, `labels.attr-include`, `labels.categories-mode`, `labels.if-speed-mode` and `labels.profile`. The `legacy` profile was never finished and is dropped; read-side continuity for history written by the other plugin is the optional #197. A `.cfg` that still sets one of the keys, renames, copies or excludes a removed label, or names `ifDescr`, `ifSpeed`, `ifHighSpeed` or `categories` in `labels.include` fails validation with a message naming the replacement; the plugin logs a validation failure at ERROR as a rejected configuration when the delivered file carries a `write.url` (before then it is a WARN, "not yet configured") and stays inactive until the line is removed, so an untouched upgrade cannot quietly emit a different schema.

### Changed

- **The metadata rows no longer repeat what the data series already carry** (#204). `onms_resource_attr` skips `nodeLabel`, `foreignSource`, `foreignId` and `location`, which every data series of the resource carries as `node_label`, `foreign_source`, `foreign_id` and `location` and no report OpenNMS ships dereferences, and the `cat_<Name>=Name` tags OpenNMS emits next to `categories`, whose wire form is the `onms_resource_category` rows. That is four to six series fewer per resource, about 900,000 on the reference fleet. A key whose label `labels.exclude` removes is a row again; `ifName` stays a row because the flow reports dereference `{ifName}`; `nodeId` stays because `node` carries `foreignSource:foreignId`; `ifDescr` and `ifAlias` stay rows as before. A change to a skipped key alone no longer triggers a re-emission, a `metadata.info-columns` column on a skipped key is rejected naming the label that carries the value, and `findMetrics` hands the skipped keys back from the labels so a custom report dereferencing one still resolves.

### Fixed

- **The interface utilisation graphs OpenNMS ships resolve `{ifSpeed}` and `{ifHighSpeed}` again** (#206). `mib2.traffic`, `mib2.traffic-inout` and `mib2.HCtraffic-inout` dereference the two strings, which the 1.0.0 wire carries only as the `onms_resource_ifspeed` gauge. `findMetrics` now issues a second query per batch for the gauge and deposits `ifHighSpeed` (the gauge in whole Mbit/s) and `ifSpeed` (the gauge capped at 4,294,967,295) as external tags, the values the SNMP collector would have stored; the wire is unchanged. For those to be exact, the gauge now prefers the exact `ifSpeed` while it is below the 32-bit cap and uses `ifHighSpeed × 1,000,000` above it, so a T1 is 1,544,000 bit/s rather than 2,000,000. `find_metrics_enrichment_batches_total` counts both queries, and both count against the 100-call cap.

### Added

- **Resource metadata on the wire** (epic #189). Three keys: `metadata.cadence-ms` (default 15 min: how often an unchanged resource's series are re-emitted; new or changed metadata goes out at once; `0` switches the series off), `metadata.attr-budget` (default 16 rows per resource, lowest keys first, remainder counted in `metadata_attrs_dropped_total`) and `metadata.info-columns` (`column=key` entries for `onms_resource_info`, default `if_alias=ifAlias, if_descr=ifDescr, resource_name=name, hr_storage_descr=hrStorageDescr, dsk_path=dskPath, datname=datname, spcname=spcname`, one column per key the shipped datacollection emits). The metadata samples go through the same queues, batches and disk tier as the data and are counted in `store_samples_offered_total`. New metrics: `metadata_series_emitted_total`, `metadata_attrs_dropped_total`, the gauge `metadata_resources`, and `find_metrics_enrichment_batches_total`. `findMetrics` restores attributes and categories from the rows with two `POST /api/v1/query` per `read.discovery-batch-size` resources under either discovery strategy (best effort, capped at 100 calls, that is 50 batches, newest emission wins after a rename), so `read.discovery-batch-size` is no longer ignored under `single-pass`. `resourceId` can no longer be excluded or renamed; it is the join key. The "apply today's alias to old data" idiom uses PromQL's `@` modifier, which needs Prometheus 2.33, any Mimir, VictoriaMetrics 1.72.0 or Cortex 1.14 (or `-querier.at-modifier-enabled` on 1.8 to 1.13); the everyday join needs nothing beyond plain PromQL, with a `last_over_time(…[30m])` on the metadata side because the series are emitted every 15 minutes.
...
- **`store()` is cheaper while a shard spills to disk** (#182). A spilled sample used to hold its shard's accept lock for a protobuf encode, a CRC and about four syscalls, and near the budget cap for a directory listing on every append, refused ones included. Now a call's samples are grouped by shard and each shard's lock is taken once per call. The calling thread encodes the frames before taking it, and a segment tracks its own size, so an append is one positional write. The bucket's byte count is tracked exactly instead of listed, so neither a refusal at the cap, nor the `overflow_bytes` gauge, nor the all-or-nothing room check lists the directory. A periodic fsync no longer holds the lock appends need. Where each sample lands is unchanged, and a property test compares the batched placement with one-at-a-time placement over generated histories.
  On a local benchmark (64 threads, 4 shards, 4 samples per call; macOS, so fsync is nearly free) the disk path went from 206,000 to 314,000 samples/sec and the memory path from 715,000 to 1,603,000. These are local figures, not fleet ones. The fleet rerun of the #182 outage is still to come.

### Fixed

- **The docs did not say what a backend outage costs OpenNMS's writer threads** (#182). A spilling shard slows `store()`, and when the writer threads cannot keep up, Horizon's ring buffer discards samples the plugin never sees. On the reference fleet under 0.8.1 that was 12.9 million samples over a 5-minute outage, 21 times what the plugin refused. Under `overflow.drain = ordered` the slow path lasts until the bucket is empty, not until the backend returns. The Drain policy section now says so and recommends `concurrent` for callers that cannot absorb it, where the backend accepts out-of-order writes. The operations guide names OpenNMS's `ring-buffer.dropped-samples` meter, to read alongside the plugin's counters when accounting for an outage.
...
- **BREAKING: the `wal.*` configuration keys** (#165, epic #168). `wal.enabled`, `wal.path`, `wal.max-size-bytes`, `wal.segment-size-bytes`, `wal.fsync` and `wal.overflow` are gone, with no aliases, and so is the validation rule that forbade `writer.shards > 1` alongside the WAL. Leftover `wal.*` lines in an existing `.cfg` are inert rather than an error. The mapping: `wal.enabled=true` becomes a non-zero `overflow.max-size-bytes` (now the default) and `wal.enabled=false` becomes `overflow.max-size-bytes=0`; `wal.path` becomes `overflow.dir`; `wal.max-size-bytes` becomes `overflow.max-size-bytes`; `wal.fsync=never` becomes `overflow.fsync=none`; `wal.overflow=backpressure|drop-oldest` becomes `overflow.full=refuse|drop-oldest`. `wal.segment-size-bytes` is removed outright — segment size is derived as an eighth of a shard's slice. `samples_dropped_wal_full_total` is replaced by `samples_dropped_overflow_full_total`.
  **Drain the 0.7.x WAL before upgrading.** Its on-disk layout is a single log at the directory root; 0.8.0 reads one subdirectory per shard and will not find it.

### Changed

- **BREAKING: the write-path defaults move to the measured configuration** (#166, epic #168): `writer.shards` 1 → 4, `queue.capacity` 10,000 → 40,000, `batch.linger-ms` 0 → 100. Arm F at the old defaults lost 7.5% of samples over eight hours with the backend idle at 0.4 of 16 cores; arm G at these accepted 99.75%.
  **This changes behaviour on upgrade with no configuration edit.** Four shards means up to four concurrent streams to the backend where there was one — check per-tenant ingest limits on Mimir and VictoriaMetrics cluster first. Per-shard gauges (`shard_<i>_queue_depth`, `shard_skew_pct`, `overflow_pending_samples_shard_<i>`) also start appearing, since they are only registered when sharding is on, so dashboards that enumerate metrics will see new series. The activation log now names the whole resolved write path on one line.
- **BREAKING: a full memory queue spills to disk instead of refusing** (#165, epic #168). Every shard now has a durable bucket behind its queue, built from the WAL machinery pointed at a per-shard directory, so sharding and durability combine for the first time. `store()` throws only when a shard's bucket is full too. WAL mode is no longer a separate pipeline: today's WAL-only deployment is the same pipeline with a minimum memory tier, and `overflow.max-size-bytes=0` is the symmetric degenerate case that keeps the pre-0.8.0 refusal behaviour for deployments that deliberately run diskless.
  When a queue fills, the shard's **whole memory backlog moves into its bucket** ahead of the refused sample, and every further sample for that shard goes to disk until the bucket drains. A queue fills from the front, so at that moment memory holds the older samples; appending only the refused one would send it ahead of them, because the bucket drains first. Moving the backlog keeps the memory queue empty for the whole spilling period, which makes disk-before-memory drain trivially correct and means every batch sent during an outage is a disk batch — retried on failure, never dropped.
  A memory batch the backend refuses is taken onto disk and retried rather than dropped, so a sample is durable from the first failure rather than only once the queue happens to fill. Batches already in flight when a shard spills are written ahead of the memory backlog, since they were offered before it, and draining the memory queue is atomic with recording the batch — otherwise a spill landing in between strands it.
  Zero loss now has a stated bound: `overflow.max-size-bytes` divided by the offered rate is how long a backend can be down before the plugin refuses. The default 4 GiB is on the order of 15–20 minutes at 64,000 samples/sec.
...
- **`overflow.drain` (`ordered` | `concurrent`, default `ordered`)** (#166, epic #168). `ordered` is the behaviour D4a shipped: a shard's bucket is drained to empty before any memory sample, and while it holds anything the shard's memory queue stays empty, so per-series order survives the tier boundary. `concurrent` keeps using memory while the bucket drains and alternates the tiers, so fresh samples do not queue behind the backlog — at the cost of per-series order across the boundary, which means it needs a backend that accepts out-of-order writes. The docs carry a per-backend table; choosing it wrongly shows up only as `samples_dropped_4xx_total`.
  Not only a drain-side knob: `ordered`'s emptiness rule lives on the accept path, so `concurrent` changes both ends.
- **`overflow_recovering_shards`** and **`overflow_oldest_pending_age_ms`** (#166). The second is the recovery figure to alert on — depth says how much is queued, which only means something against the drain rate, where this says how far behind the tier is in the unit the latency budget uses. Correct from the moment a recovered bucket opens, before anything has been drained.
- **`overflow.dir`, `overflow.max-size-bytes` (4 GiB), `overflow.full` (`refuse` | `drop-oldest`, default `refuse`) and `overflow.fsync` (`always` | `batch` | `none`, default `batch`)** (#165, epic #168) configure the disk tier. `refuse` is the default because silent loss should be a deliberate choice; `drop-oldest` evicts a whole oldest segment and keeps accepting, discarding it whether or not the backend has taken it — during an outage the checkpoint is frozen, so sparing unacknowledged data would make the policy a no-op precisely when it is wanted.
- **`samples_spilled_total`**, **`samples_drained_from_overflow_total`**, **`samples_dropped_overflow_full_total`**, **`samples_evicted_overflow_total`**, and the gauges **`overflow_pending_samples`** (total and `overflow_pending_samples_shard_<i>` per shard) and **`overflow_bytes`** (#165). The reconciliation identity gains a term: offered = written + unmapped + Σ dropped + queue_depth + overflow_pending, exact between scrapes rather than at every instant. Per-shard pending is what shows hash skew filling one bucket while its siblings idle.
...
- **`opennms:prometheus-writer-stats` never registered** (#113).
  ...
  The e2e smoke gates execute the command over Karaf SSH, so a registration regression fails CI loudly instead of rotting silently.
  As a consequence, the per-shard gauges introduced by the sharded write pipeline now actually reach operators through the command's table.
...
[Unreleased]: https://github.com/labmonkeys-space/prometheus-remote-writer/compare/v1.0.3...HEAD
[1.0.3]: https://github.com/labmonkeys-space/prometheus-remote-writer/compare/v1.0.2...v1.0.3
